    ...
    try {
      if (tipo === 'produtos') {
        const produtos = dados.map(row => {
          // Garantir que estoque_minimo sempre seja um número válido
          let estoqueMinimo = 5; // valor padrão
          if (row.estoque_minimo && row.estoque_minimo.trim() !== '') {
            const parsed = Number(row.estoque_minimo);
